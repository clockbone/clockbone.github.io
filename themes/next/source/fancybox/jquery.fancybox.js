!function (e, t, i, n) {
    ...
    i.extend(s, {
        ...
        _setDimension: function () {
            var e, t, n, o, a, r, l, c, d, p, h, u, y, x, v, w = s.getViewport(), b = 0, k = !1, C = !1, O = s.wrap, W = s.skin, _ = s.inner, S = s.current, T = S.width, E = S.height, L = S.minWidth, H = S.minHeight, P = S.maxWidth, R = S.maxHeight, j = S.scrolling, M = S.scrollOutside ? S.scrollbarWidth : 0, A = S.margin, I = g(A[1] + A[3]), D = g(A[0] + A[2]);
            ...
            if (T = g(a), E = g(r), d = a / r, L = g(f(L) ? g(L, "w") - n : L), P = g(f(P) ? g(P, "w") - n : P), H = g(f(H) ? g(H, "h") - o : H), R = g(f(R) ? g(R, "h") - o : R), l = P, c = R, S.fitToView && (P = Math.min(w.w - n, P), R = Math.min(w.h - o, R)), u = w.w - I, y = w.h - D, S.aspectRatio ? (T > P && (T = P, E = g(T / d)), E > R && (E = R, T = g(E * d)), T < L && (T = L, E = g(T / d)), E < H && (E = H, T = g(E * d))) : (T = Math.max(L, Math.min(T, P)), S.autoHeight && "iframe" !== S.type && (_.width(T), E = _.height()), E = Math.max(H, Math.min(E, R))), S.fitToView)if (_.width(T).height(E), O.width(T + e), p = O.width(), h = O.height(), S.aspectRatio)for (; (p > u || h > y) && T > L && E > H && !(b++ > 19);)E = Math.max(H, Math.min(R, E - 10)), T = g(E * d), T < L && (T = L, E = g(T / d)), T > P && (T = P, E = g(T / d)), _.width(T).height(E), O.width(T + e), p = O.width(), h = O.height(); else T = Math.max(L, Math.min(T, T - (p - u))), E = Math.max(H, Math.min(E, E - (h - y)));
            M && "auto" === j && E < r && T + e + M < u && (T += M), _.width(T).height(E), O.width(T + e), p = O.width(), h = O.height(), k = (p > u || h > y) && T > L && E > H, C = S.aspectRatio ? T < l && E < c && T < a && E < r : (T < l || E < c) && (T < a || E < r), i.extend(S, {
                dim: {
                    width: m(p),
                    height: m(h)
                },
            ...
            }), !x && S.autoHeight && E > H && E < R && !C && _.height("auto")
        },
